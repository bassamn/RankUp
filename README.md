...
Outputs are written to `release/`: an unsigned NSIS `.exe` on Windows, `.dmg` and `.zip` on macOS, and `.AppImage` plus `.deb` on Linux. Build the renderer with `npm run build` before invoking a platform package command. macOS installers must be built on macOS; Windows installers are best built on Windows.
